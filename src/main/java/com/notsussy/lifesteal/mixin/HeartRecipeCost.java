 ...
 * 3:TrialKey 4:Totem    5:TrialKey
 * 6:Diamond  7:Emerald  8:Diamond
 * </pre>
 *
 * A vanilla shaped recipe only ever requires (and consumes) exactly 1 item per grid
 * slot, so the diamond slots' extra cost beyond that is enforced separately:
 * {@link CraftingMenuMixin} blocks the result unless every diamond slot already
 * holds enough, and {@link ResultSlotMixin} removes the rest on top of vanilla's own
 * 1 when the item is actually taken.
 */
